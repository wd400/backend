...
# mongosh --authenticationDatabase admin -u "$MONGODB_ROOT_USER" -p "$MONGODB_ROOT_PASSWORD"<<EOF
# use admin
# db.createUser(
#     {
#         user: "beats",
...
# db.banned.createIndex({"id":1});

# EOF

printf aaaaaaaaaaaa
mongorestore -u "$MONGODB_ROOT_USER" -p "$MONGODB_ROOT_PASSWORD" --archive=/tmp/db.bson
